      ...
      <div class="col-md-12">
        <div class="card card-Primary">
          <div class="card-header">
            <h3 class="card-title">Payment Receive Report</h3>
          </div>
        
          <form method="post" action="{{route('PaymentReceiveReport.post')}}">
          	@csrf
            <div class="card-body">
            	<div class="row">
            		<div class="col-md-3">
		...
		<div class="container-fluid">
		    ...
		    <div class="card-body">
		      <table id="example1" class="table table-bordered table-striped">
		        ...
		        <tbody>
		        	<?php $i=1;  $from_date = date_format(date_create($from_date),"Y-m-d");
            $to_date = date_format(date_create($to_date),"Y-m-d"); ?>
		        	@foreach($main as  $data)
		          <tr>
		            <td>{{$data['website_name']}}</td>
		            <td>{{$data['data'][0]['receive_amount']}}</td>
		          	<td><a href="{{route('receive_payment_data',['website'=>$data['website_id'],'from_date'=> $from_date,'to_date'=>$to_date])}}"><i class="fa fa-eye" ></i></a></td>
		          </tr>
		          <?php $i++;?>
		          @endforeach
		        </tbody>

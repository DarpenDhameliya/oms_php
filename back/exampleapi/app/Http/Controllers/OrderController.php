...
class OrderController extends Controller
{
	public function orderentry(Request $request){
		if(Auth::check()){
            ...
            session()->put('sub_menu',"Manual_Order_Entry");
	...
	public function Import_Flipcartexcel(){
		if(Auth::check()){
            ...
            $user = Auth::user();
            session()->put('mainmenu',"Order_Entry");
			session()->put('sub_menu',"Import_Flipcartexcel");
    ...
    public function import(Request $request){
      ...
      /* print_r($request->file->getClientMimeType());
       die();*/
	...
	public function Import_amazoneexcel(){
		if(Auth::check()){
            session()->put('mainmenu',"Order_Entry");
			session()->put('sub_menu',"Import_amazoneexcel");
	...
	public function Import_meeshoexcel(){
		if(Auth::check()){
            session()->put('mainmenu',"Order_Entry");
			session()->put('sub_menu',"Import_meeshoexcel");
    ...
    public function order_return(){
        if(Auth::check()){
            session()->put('mainmenu',"Order_Return");

            $order_status = \Config::get('data.order_status');
            $return_reason = Return_reason::get();

            return view('fronted.order-return',compact('order_status','return_reason'));
        }
        return redirect("login");
    }
    ...
    public function get_returndata(Request $request){
        if(Auth::check()){
            ...
            if(!sizeof($query)){
                ...
                if(!sizeof($query)){
                    return response()->json(["result"=>false,'error'=>'Order in Return Status Already.']);
                }else{
                    $result = OrderEntry::select('order.*','courier.name as courier_name','website.name as website_name','product.name as product_name')->join('courier', 'courier.id', '=', 'order.courier_id','left')
                            ->join('website', 'website.id', '=', 'order.website_id')
                            ->join('product', 'product.id', '=', 'order.product_id')
